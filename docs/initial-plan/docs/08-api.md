...
Behavior:
- Existing clip with no prior processing job: returns `202` and enqueues one processing message.
- Unknown clip: returns `404`.
- Duplicate identical request: returns `409 conflict` (safe rejection; does not enqueue again).
