...
- `clips`
  ...
  - `title` — required creator-provided title used in portal listings and review queues
...
- `processing_jobs`
  ...
  - `stage` — enum: `audio`, `asr`, `segment`, `translate`, `finalize`
...
- `segment_translations`
  ...
  - `segment_index` — FK-like alignment to `transcript_segments.segment_index`
  - `source_text` — Thai segment text used for translation
  - `english_text` — English segment translation for review
...
- Segment translations stay aligned by `segment_index` within a clip version.

---

## translation-provider-config

Segment translation uses OpenRouter structured outputs for whole-clip Thai -> English translation.

Storage:
- `OPENROUTER_API_KEY` and `OPENROUTER_TRANSLATION_MODEL` are stored in environment variables
- generated translation artifacts are written to R2 alongside segment JSON

Operational note:
- the legacy local `@huggingface/transformers` backend remains available in code for rollback
